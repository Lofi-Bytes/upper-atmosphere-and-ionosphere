...
def T_new(T, sumQ, delta_z, lam_n, dt, RHO, Cp):
    ...
    A[998] = 1.0
    B = np.zeros(1000.0) - 2.0 - (RHO*Cp*np.power(delta_z, 2))/(lam_n*dt) ; B[0] = -1.0  ;  B[999] = -1.0
    C = np.zeros(999.0) + 1.0 
    for i in range(999):
        C[i] -= dlam_n[i]/(4.0*lam_n[i])
    C[0] = 0.0
    D = np.zeros(1000.0) - (np.power(delta_z,2)/lam_n)*(T*(RHO*Cp/dt) + sumQ) ; D[0] = -200.0 ; D[999] = 0.0
...
def ion_production_rate(n, I, data):
    Ps = np.zeros(1000.0)
    for s in ions:
        for i in range(1000.0):
            for j in range(37):
                Ps[s][i][j] += n[i]*I[i][j]*data[s][j]
    return Ps
