...
theta = np.linspace(0, np.pi, zsize)                                    # Theta ranges from 0 [rad] - pi [rad] in incriments of 1000. 
...
days = 15                                                               # Number of days you want to calculate over.
...
def calculate_average_scale_height(n_base, m, H, rho_tot):    
    ...
    # the neutral species from the base, z(0), up to an altitude of 120km, z(29).
    H_ave = np.zeros(29.0)
    for i in range (0, 29):
...
def read_solar_flux():    
    ...
    data['O_plus'] = data['O4S'] + 2*data['O2_Op'] + data['O2D'] + data['O2P']
    #Summing up the various N+ souces to get the total amount of N+
    data['N_plus'] = 2*data['N2_Np'] + data['N_Np']
    # The absorption cross section values for each of the neutral species in the table 
...
def combine_energy_dissipation(Q):
    # To calculate the TOTAL heating over all wavelengths for all species we must sum
...
def T_new(T, sumQ, delta_z, lam_n, dt, RHO, Cp):
    #Defineing the thermal conduction in [W/(m*K)].
    lam_n = 3.6E-4*pow(T, .75)
    dlam_n = np.zeros(1000)
    dlam_n[1:998] = lam_n[2:999] - lam_n[0:997]
    dlam_n[0] = dlam_n[1]
    dlam_n[999] = dlam_n[998]
    A = np.zeros(999) + 1.0 ; A[998] = 1.0
    for i in range(999):
        A[i] += dlam_n[i]/(4.0*lam_n[i]) 
    B = np.zeros(1000) - 2.0 - (RHO*Cp*np.power(dz, 2))/(lam_n*dt) ;  B[0] = -1.0  ;  B[999] = -1.0
    C = np.zeros(999) + 1.0 ; C[0] = 0.0
    for i in range(999):
        C[i] -= dlam_n[i]/(4.0*lam_n[i]) 
    D = np.zeros(1000) - (np.power(dz,2)/lam_n)*(T*(RHO*Cp/dt) + sumQ) ; D[0] = -200.0 ; D[999] = 0.0
...
'''
def T_new(T, sumQ, dz, lam_n, dt, RHO, Cp, ztop):
#    for loop in range(10):
    # Defineing the delta change in thermal conduction in [W/(m*K)].
    dlam_n = np.zeros(ztop+1)
    dlam_n[1:ztop-1] = lam_n[2:ztop] - lam_n[0:ztop-2]
    dlam_n[0] = dlam_n[1]
    dlam_n[ztop] = dlam_n[ztop-1]
    # Setting up the coeficients of the tri diagonal matrix.
    A = np.zeros(ztop+1) + 1.0 + dlam_n/(4.0*lam_n) ; A[998] = 1.0
    B = np.zeros(ztop+1) - 2.0 - (RHO*Cp*np.power(dz, 2))/(lam_n*dt) ;  B[0] = -1.0  ;  B[999] = -1.0
    C = np.zeros(ztop+1) + 1.0 - dlam_n/(4.0*lam_n) ; C[0] = 0.0
    D = np.zeros(ztop+1) - (np.power(dz,2)/lam_n)*(T*(RHO*Cp/dt) + sumQ) ; D[0] = -200.0 ; D[999] = 0.0
    coef = np.zeros([1000,1000])
#    coef[1:ztop,0:ztop-1] = A[0:ztop-1]
#    coef[0:ztop,0:ztop] = B[0:ztop]
#    coef[0:ztop-1,1:ztop] = C[1:ztop]
    for i in range(999):
        coef[i+1,i] = A[i]
    for i in range(1000):
        coef[i,i] = B[i]
