...
days = 0.5                                                                # Number of days you want to calculate over.
dt = 60.0                                                              # Definin time incriment as 300 [s].
...
for loop in range(tloop):
    ...
    I = ip.calculate_inensity(I, I_inf, sumtau)
...
#        GOm['O+'][i] = (ni['O+'][i] - ni['O+'][i-1])/dz[i]
#    GOm['O+'][0] = 0.0
    
#    for i in range(0,999):
#        GOp['O+'][i] = (ni['O+'][i+1] - ni['O+'][i])/dz[i]
#    GOp['O+'][999] = 0.0    
    
    
#    for i in range(0,999):
#        GOm['O+'][i+1] = (ni['O+'][i+1] - ni['O+'][i-2])/dz[i]    
#        GOp['O+'][i] = (ni['O+'][i+1] - ni['O+'][i])/dz[i]
#        GOm['O+'][0] = 0.0
#        GOp['O+'][999] = 0.0        
#        dr1[i] = -dt * (vp * GOm['O+'][i] + vm * GOp['O+'][i])
#        ni['O+'][i] = ni['O+'][i] + dr1[i]
    
    #First order
    GOm['O+'][1:-1] = (ni['O+'][1:-1] - ni['O+'][0:-2])/dz[1:-1]
    GOm['O+'][0] = 0.0
    GOp['O+'][0:-2] = (ni['O+'][1:-1] - ni['O+'][0:-2])/dz[0:-2]
    GOp['O+'][-1] = 0.0
    
    dr1 = -dt * (vp * GOm['O+'] + vm * GOp['O+'])
    Oplus_old = ni['O+']
    ni['O+'] = ni['O+'] + dr1
    
    # Calculating electron temperatures separately.
    Te = ip.Te(Te, sumQ, dz, eps, eps_e, ni_sum, N_tot)    
...
Assignment 4/Figures/B/Ion_Production_Rate_vs_Altitude_100_800.eps')
...
ax2.legend(['O+','N+', 'O2+', 'N2+', 'O+$_{old}$'])
fig2.savefig('/Users/jsni/Desktop/Ionospheres Assignments/\
Assignment 4/Figures/B/Ion_Number_Density_vs_Altitude_100_800.eps')
...
Assignment 4/Figures/B/Ion_Number_Density_vs_time.eps')
...
Assignment 4/Figures/B/Total_Ion_Number_Density_vs_Altitude_100_800.eps')

'''
# Maximum Temperature vs Time
fig5 = plt.figure(figsize=[12.15,6.06])
ax5 = fig5.add_subplot(111)
ax5.set_title('Maximum Temperature vs Time')
ax5.set_xlabel('Time $[days]$')
ax5.set_ylabel('Temperature $[K]$')
ax5.plot(time/dayseconds, T_max, time/dayseconds, Te_max)
ax5.legend(['T','T$_e$'])
fig5.savefig('/Users/jsni/Desktop/Ionospheres Assignments/\
Assignment 4/Figures/B/T_max_vs_time.eps')
'''
